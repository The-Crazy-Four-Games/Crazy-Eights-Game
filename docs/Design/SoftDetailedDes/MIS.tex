...
\subsection{Uses}
\begin{itemize}
    \item Matchmaking Module (M3) \hyperref[sec:matchmaking-module]{M3}
    \item Authentication Module (M4) \hyperref[sec:authentication-module]{M4}
    \item Repository Module (M5) \hyperref[sec:repository-module]{M5}
\end{itemize}
...
\begin{itemize}
    \item Authentication Module (M4) \hyperref[sec:authentication-module]{M4}
    \item Game Actions Module (M19) \hyperref[sec:game-actions-module]{M19}
    \item Matchmaking Module (M3) \hyperref[sec:matchmaking-module]{M3}
\end{itemize}
...
\begin{itemize}
    ...
    \item output: Calls \texttt{emitGameState(socket, currentState)} to send the current state to the joining player.
...
\subsection{Uses}
\begin{itemize}
    \item Database Module \hyperref[sec:database-module]{M22 (PostgreSQL)}
\end{itemize}
...
\subsection{Uses}
\begin{itemize}
    \item Operating System Module \hyperref[sec:os-module]{M20 (Server OS)}
\end{itemize}
...
\subsection{Uses}
\begin{itemize}
    \item M9 Game State Manager Module
    \item M15 Game Engine Module
    \item M16 Rules Module
    \item M17 Scoring Module
\end{itemize}
...
\midrule
validateMove & Card, PlayerID & Boolean & InvalidMoveException \\
executeMove & Card, PlayerID & GameState & GameStateError \\
drawCard & PlayerID & Card & EmptyDeckException \\
declareSuit & Suit & None & InvalidDeclarationException \\
\bottomrule
\end{tabular}
\end{center}
...
\subsubsection{State Variables}
\begin{itemize}
    \item \textbf{currentPlayer}: the player whose turn is active.
    \item \textbf{currentState}: snapshot of the latest game state.
\end{itemize}

\subsubsection{Environment Variables}
\begin{itemize}
    \item Game state data received from the server and Rules Module; user input events from UI.
\end{itemize}

\subsubsection{Assumptions}
\begin{itemize}
    \item Game state remains synchronized between client and server.
    \item Player actions are sequential and unique per turn.
\end{itemize}

\subsubsection{Access Routine Semantics}
\noindent\textbf{validateMove}(\textit{Card, PlayerID})
\begin{itemize}
    \item output: checks if the card follows Crazy Eights rules (suit/rank match or valid wild).
\end{itemize}

\noindent\textbf{executeMove}(\textit{Card, PlayerID})
\begin{itemize}
    \item transition: applies move results and notifies dependent modules of updates.
    \item output: \texttt{GameState}.
\end{itemize}

\noindent\textbf{drawCard}(\textit{PlayerID})
\begin{itemize}
    \item transition: requests one card from the draw pile and adds it to the player’s hand.
    \item output: \texttt{Card}.
\end{itemize}

\noindent\textbf{declareSuit}(\textit{Suit})
\begin{itemize}
    \item transition: processes suit declaration after an “8” is played.
\end{itemize}

\subsubsection{Local Functions}
\begin{itemize}
    \item \texttt{computeNextTurn()}: determines which player should act next.
\end{itemize}

\subsubsection{Considerations}
\begin{itemize}
    \item This module must ensure that all moves are deterministic and reversible for testing and replay.
\end{itemize}
...
\begin{itemize}
    \item M9 Game State Manager Module
    \item M17 Scoring Module
    \item M18 Base Conversion Module
\end{itemize}
...
\subsection{Uses}
\begin{itemize}
    \item M12 Move Controller Module
    \item M16 Rules Module
    \item M17 Scoring Module
    \item M9 Game State Manager Module
\end{itemize}
...
\begin{itemize}
    \item M15 Game Engine Module
    \item M18 Base Conversion Module
    \item M13 Scoreboard View Module
\end{itemize}
...
\subsubsection{Environment Variables}
\begin{itemize}
    \item Player input (from Move Controller), and Game State object.
\end{itemize}

\subsubsection{Assumptions}
\begin{itemize}
    \item Each action follows the command pattern and can be validated independently.
    \item The game engine ensures single-threaded execution for action safety.
\end{itemize}
...
\begin{itemize}
    \item output: checks if the action is allowed under current rules.
\end{itemize}

\noindent\textbf{executeAction}(\textit{Action, GameState})
\begin{itemize}
    \item transition: applies changes to game state and notifies observers.
    \item output: \texttt{GameState}.
\end{itemize}
...
\subsection{Uses}
\begin{itemize}
    \item M21 Browser Runtime Module
    \item M22 Database Module
\end{itemize}
...
\subsection{Uses}
\begin{itemize}
    \item M20 Operating System Module
    \item M22 Database Module
\end{itemize}
