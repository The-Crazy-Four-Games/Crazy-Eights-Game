...
\multirow{4}{0.3\textwidth}{Behaviour-Hiding Module} & (Core Domain Logic) & \mref{mGameEngine} \\
& & \mref{mRules} \\
& & \mref{mScoring} \\
...
\begin{description}
  ...
  \item[Access Programs:] `createGame(players, options)` $\to$ `GameState`; `applyMove(gameState, move)` $\to$ `GameState`; `checkWinCondition(gameState)` $\to$ `PlayerID | null`
...
\begin{description}
  ...
  \item[Access Programs:] `on('connection', socket)`, `on('joinGame', data)`, `on('submitMove', data)`, `emit('gameStateUpdate', state)`
...
\begin{description}
  ...
  \item[Access Programs:] `connect()`, `disconnect()`, `on('gameStateUpdate', callback)`, `emit('submitMove', move)`
...
\begin{description}
  ...
  \item[Access Programs:] `onCardClick(card)`, `onDrawClick()`
...
FR-2 Turn management & \mref{mGameEngine}, \mref{mRules}, \mref{mRealtimeGateway}, \mref{mRealtimeClient}, \mref{mGameBoardView}\\
FR-3 Rule validation & \mref{mRules}, \mref{mGameEngine}, \mref{mMoveController}, \mref{mGameBoardView}\\
FR-4 Special cards & \mref{mRules}, \mref{mGameEngine}, \mref{mGameBoardView}\\
FR-5 End of game & \mref{mGameEngine}, \mref{mScoring}, \mref{mScoreboardView}, \mref{mRepository}\\
FR-6 Calculate score & \mref{mScoring}, \mref{mBaseConversion}, \mref{mScoreboardView}\\
